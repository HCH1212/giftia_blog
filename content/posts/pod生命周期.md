...
stateDiagram-v2
    [*] --> 容器启动
    容器启动 --> 启动探针: startupProbe 生效
    启动探针 --> 就绪检查: startupProbe 成功
    启动探针 --> 容器启动: startupProbe 失败(重试)
    
    state 运行中 {
        就绪检查 --> 接收流量: readinessProbe 成功
        接收流量 --> 摘除流量: readinessProbe 失败
        摘除流量 --> 接收流量: readinessProbe 恢复
        --
        接收流量 --> 重启容器: livenessProbe 失败
        摘除流量 --> 重启容器: livenessProbe 失败
    }
